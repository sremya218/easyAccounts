...
from administration.models import Salesman, BonusPoint
...
class DeliveryNote(models.Model):
    ...
    def get_json_data(self):
        ...
        deliverynote_data = {
            ...
            'deliverynote_invoice_date' : self.deliverynote_invoice_date.strftime('%d/%m/%Y'),
...
class DeliverynoteItem(models.Model):
    ...
    def get_json_data(self):
        ...
        deliverynote_item_data = {
            ...
            'uoms': self.batch_item.get_json_data()['uoms'],
            'stock': self.batch_item.get_json_data()['stock'],
            'stock_unit': self.batch_item.get_json_data()['stock_unit'],
            ...
            'purchase_unit': self.batch_item.uom,
...
class Sale(models.Model):
    ...
    def get_json_data(self):
        ...
        tax_exclusive_total = self.grant_total - self.sales_tax
        sales_data = {
            'id': self.id,
...
class SalesItem(models.Model):
    ...
    def get_json_data(self):
        ...
        if self.sales.bill_type == 'Invoice':
            tax_exclusive_amount = float(self.net_amount) - (float(self.net_amount) * float((self.batch_item.item.vat_type.tax_percentage)/100))
        else:
            tax_exclusive_amount = float(self.net_amount)
        sales_item_data = {
            'id': self.id,
            'item_id': self.batch_item.item.id,
            'code': self.batch_item.item.code,
...
class Estimate(models.Model):
    ...
    def get_json_data(self):
        e_items = []
        for e_item in self.estimateitem_set.all():
            e_items.append(e_item.get_json_data())
        estimate_data = {
            'estimate_id' : self.id,
            'bill_type':self.bill_type,
            'estimate_invoice_date' : self.estimate_invoice_date.strftime('%d/%m/%Y'),
            'customer_id': self.customer.id if self.customer else '',
            'salesman_id': self.salesman.id if self.salesman else '',
            'customer': self.customer.name if self.customer else '',
            'salesman': self.salesman.first_name+ '' +self.salesman.last_name if self.salesman else '',
...
class SalesReturn(models.Model):
    ...
    def get_json_data(self):
        s_items = []
        for s_item in self.salesreturnitem_set.all():
            s_items.append(s_item.get_json_data())
        ...
        sales_return_data = {
            ...
            'grant_total': self.grant_total,
            'customer': self.sales.customer.name if self.sales.customer else '',
            'salesman':self.sales.salesman.first_name + '' +self.sales.salesman.last_name if self.sales.salesman else '',
            'transaction_reference_no': self.transaction_reference_no,
            'sales_invoice': self.sales.sales_invoice_number,
            'payment_mode': self.payment_mode,
            'bank_name': self.bank_name,
            'items':s_items,
            'bill_type': self.sales.bill_type,
            'do_no': self.sales.do_number,
            'card_number': self.card_number,
            'cheque_date': self.cheque_date.strftime('%d/%m/%Y') if self.cheque_date else '',
            'cheque_number': self.cheque_number,
            'branch': self.branch,
            'card_holder_name': self.card_holder_name,
            'round_off': self.sales.round_off,
        }
...
class SalesReturnItem(models.Model):
    ...
    def get_json_data(self):
        ...
        sales_return_item_data = {
            'id': self.id,
            'code': self.sales_item.batch_item.item.code,
            'name': self.sales_item.batch_item.item.name,
            'batch': self.sales_item.batch_item.batch.name,
            'item_quantity': self.quantity,
            'uom': self.uom,
            'net_amount': self.net_amount,
            'tax': self.sales_item.batch_item.item.vat_type.tax_percentage if self.sales_item.batch_item.item.vat_type else '',
            'mrp': self.sales_item.mrp,
        }
        return sales_return_item_data
