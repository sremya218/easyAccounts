...
from sales.models import SalesItem, Sale,Invoice, Estimate, EstimateItem,  Receipt, SalesReturn, SalesReturnItem, EditedInvoiceSale, EditedInvoiceSaleItem, EditedReceipt, EditedInvoice, DeliveryNote, DeliverynoteItem
from inventory.models import Item, BatchItem, Batch,StockValue, Brand, Category, VatType
...
font_path_regular = settings.PROJECT_ROOT.replace("\\", "/")+"/arabic-fonts/AdobeArabic-Regular.ttf"
font_path_bold = settings.PROJECT_ROOT.replace("\\", "/")+"/arabic-fonts/AdobeArabic-Bold.ttf"
pdfmetrics.registerFont(TTFont('Arabic-normal', font_path_regular))
pdfmetrics.registerFont(TTFont('Arabic-bold', font_path_bold))


def normal_receipt(request):
    ...
    if request.GET.get('transaction_ref_no'):
        ...
        data = []
        data.append(['Sold To:', sale.customer.name if sale.customer else 'Counter Sales'])
        ...
        if sale.payment_mode == 'credit':
            data.append(['Paid',sale.paid])
            data.append(['Balance',sale.balance])
        if sale.bill_type == 'Invoice':
...
def arabicreceipt(request):
    ...
    customer_name = ''
    customer_name = sales.customer.name if sales.customer else "Counter Sales"
...
class SalesEntry(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if sales_details['payment_mode'] == 'credit':
                if sales_details['Paid'] != '':
                    sale.paid = sales_details['Paid']
                    sale.balance=sales_details['balance']
            ...
            for item in sales_items:
                ...
                sales_item.item = item_obj
                quantity = item['quantity']
                quantity_unit = item['uom']
                actual_quantity = calculate_actual_quantity(item_obj, quantity, quantity_unit)
                if item['type'] == 'Stockable':
                    print batch_item_obj.quantity_in_actual_unit
                    print actual_quantity
                    batch_item_obj.quantity_in_actual_unit = batch_item_obj.quantity_in_actual_unit - float(actual_quantity)
                    batch_item_obj.save()
                sales_item.save()
            ...
            if sales_details['customer']:
                ...
                debit_ledger = counter_sales_ledger
               
            ledger_entry_debit_customer = create_ledger_entry(debit_ledger, sale.sales_invoice_date, sale.transaction_reference_no, sale.grant_total, None)
            
            sales_ledger = Ledger.objects.get(name='Sales')
            
            if sales_details['tax_exclusive_total'] != '':
               
                ledger_entry_credit_sales = create_ledger_entry(sales_ledger, sale.sales_invoice_date, sale.transaction_reference_no, None, sales_details['tax_exclusive_total'])
               
                sales_ledger.balance = float(sales_ledger.balance) - float(ledger_entry_credit_sales.credit_amount)
                sales_ledger.save()
            if ledger_entry_credit_sales:
                transaction_1 = create_transaction(ledger_entry_debit_customer, ledger_entry_credit_sales)
            else:
                transaction_1 = create_transaction(ledger_entry_debit_customer, None)
            ...
            if sale.payment_mode != 'credit':
                ...
                ledger_entry_debit_accounts = create_ledger_entry(debit_ledger, sale.sales_invoice_date, sale.transaction_reference_no, sale.grant_total, None)
                
                if sales_details['customer']:
                    customer = Customer.objects.get(id=sales_details['customer'])
                    credit_ledger = customer.ledger
                    
                else:
                    counter_sales_ledger = Ledger.objects.get(name="Counter Sales")
                    credit_ledger = counter_sales_ledger
                    
                ledger_entry_credit_customer = create_ledger_entry(credit_ledger, sale.sales_invoice_date, sale.transaction_reference_no, None, sale.grant_total)
                ...
                transaction_2 = create_transaction(ledger_entry_debit_accounts, ledger_entry_credit_customer)
            if sale.payment_mode != 'credit':
            ...
            ledger_entry_credit_stock = create_ledger_entry(credit_stock_ledger, sale.sales_invoice_date, sale.transaction_reference_no, None, float(sale.grant_total) - float(sale.sales_tax))
            ...
            transaction_3 = create_transaction(None, ledger_entry_credit_stock)
            if sale.payment_mode == 'credit':
            ...
            if sales_details['bill_type'] == 'Invoice':
                ...
                ledger_entry_credit_tax_account = create_ledger_entry(credit_tax_ledger, sale.sales_invoice_date, sale.transaction_reference_no, None, float(sale.grant_total) - float(sales_details['tax_exclusive_total']))
                ...
                transaction_4 = create_transaction(None, ledger_entry_credit_tax_account)
...
class SalesView(View):
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            sales_data = {}
            if customer != 'undefined' and invoice:
                sale = Sale.objects.get(sales_invoice_number=invoice, customer__ledger__id=customer, payment_mode='credit')
            else:
                receipts = []
                invoices = []
                if sales_invoice:
...
class SaleReturn(View):
        ...
        if request.is_ajax():
            ...
            if sale:
                ...
                sales_details.append({
                    ...
                    'customer': sale.customer.name if sale.customer else 'Counter Sales',
    ...
    def post(self, request, *args, **kwargs):
        ...
        for item in sales_return_items:
            if item['returned_qty'] != '' and float(item['returned_qty']) != 0 and item['type'] != 'Services':
                ...
                if item['type'] == 'Stockable':
                    ...
                    actual_quantity = calculate_actual_quantity(batch_item.item, return_item.quantity, return_item.uom)
        ...
        debit_sales_return_entry = create_ledger_entry(sales_return_ledger, sales_return.invoice_date, sales_return.transaction_reference_no, float(sales_return.grant_total) - float(total_tax_amount), None)
        print sales_return_ledger.balance, debit_sales_return_entry.debit_amount
        ...
        credit_customer_entry = create_ledger_entry(credit_ledger, sales_return.invoice_date, sales_return.transaction_reference_no, None, sales_return.grant_total)
        ...
        transaction_1 = create_transaction(debit_sales_return_entry, credit_customer_entry)
        ...
        debit_stock_entry = create_ledger_entry(stock_ledger, sales_return.invoice_date, sales_return.transaction_reference_no, stock_amount_value, None)
        ...
        transaction_2 = create_transaction(debit_stock_entry, None)
        ...
        if sales_return_details['bill_type'] == 'Invoice':
            ...
            debit_tax_account_entry = create_ledger_entry(debit_tax_ledger, sales_return.invoice_date, sales_return.transaction_reference_no, total_tax_amount, None)
            ...
            credit_cash_bank_entry = create_ledger_entry(credit_ledger, sales_return.invoice_date, sales_return.transaction_reference_no, None, total_tax_amount)
            ...
            # transaction_3.credit_ledger = credit_cash_bank_entry
            # transaction_3.debit_ledger = debit_tax_account_entry
            transaction_3 = create_transaction(debit_tax_account_entry, credit_cash_bank_entry)
        ...
        if request.is_ajax():
            ...
            try:    
                ...
                try:
                    ...
                    return HttpResponse(response, status=200, mimetype='application/json')
            if sales_return:
                sales_details = sales_return.get_json_data()
                
                res = {
                    'sales_view': sales_details,
                    'result': 'ok',
                }
            else:
                res = {
        ...
        if request.is_ajax():
            ...
            try:
                ...
                for item in estimate_items:
                    ...
                    estimate_item.uom = selling_unit
                    estimate_item.price_type = item['price_type']
...
class DeliverynoteEntry(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        current_date = datetime.now().date()
        return render(request, 'delivery_note.html', {'current_date': current_date.strftime('%d/%m/%Y'),})
    def post(self, request, *args, **kwargs):
        ...
        if request.is_ajax():
            ...
            if estimate_invoice_number:
                try:
                    ...
                    estimate_details = estimate.get_json_data()
                    res = {
    ...
    def get(self, request, *args, **kwargs):
        ...
        if vat_id:
            ...
            if request.is_ajax():
                ...
                heading = 'Tax Wise Sales Report - '+vat_type.vat_type
                d = [[heading]]
                t = Table(d, colWidths=(450), rowHeights=25, style=style)
                t.setStyle([('ALIGN',(0,0),(-1,-1),'CENTER'),
                            ('TEXTCOLOR',(0,0),(-1,-1),colors.black),
            ...
            elements.append(Spacer(2,20 ))
            data = []
            data.append(['Invoice', 'Date', 'Transaction', 'VatType', 'Tax', 'Grant Total'])
            table = Table(data, colWidths=(50, 60, 80, 80, 80, 80), style=style)
            table.setStyle([
                        ('FONTSIZE', (0,0), (-1,-1), 10),
                        ])   
            ...
            for vat_type in vat_types:
                ...
                for item in items:
                    try:
                        ...
                        for sales_item in sales_items:
                            data.append([sales_item.sales.sales_invoice_number, sales_item.sales.sales_invoice_date.strftime('%d/%m/%Y'),sales_item.sales.transaction_reference_no, \
                            item.vat_type.vat_type + str(item.vat_type.tax_percentage) if item.vat_type else '', sales_item.sales.sales_tax, sales_item.net_amount])
                            # sales_item_details.append({
                            #     'vat_type':item.vat_type.vat_type + str(item.vat_type.tax_percentage) if item.vat_type else '' ,
                            #     'invoice_no': sales_item.sales.sales_invoice_number,
                            #     'invoice_date': sales_item.sales.sales_invoice_date.strftime('%d/%m/%Y'),
                            #     'transaction_reference_no': sales_item.sales.transaction_reference_no,
                            #     'discount': sales_item.sales.discount,
        ...
        return render(request, 'tax_wise_sales_report.html', {})
            ...
            for area in area_list:
                ...
                for customer in customers:  
                    ...
                    if customer.area:
                        ...
                        table1 = Table(data1, colWidths=(100,100,100), style=style)
                        table1.setStyle([
                                    ('FONTSIZE', (0,0), (-1,-1), 10),
                                    ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
                                    ('INNERGRID', (0,0), (-1,-1), 0.25, colors.black),
...
class DeliveryNoteToSales(View):
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            if deliverynote_invoice_number:
                try:
                    delivery = DeliveryNote.objects.get(deliverynote_invoice_number = deliverynote_invoice_number,is_converted=False)
                    res = {
                        'result': 'ok',
                        'delivery_note':delivery.get_json_data(),
                    }
                except Exception as ex:
                    print str(ex)
                    res = {
...
class EditSales(View):
    ...
    def post(self, request, *args, **kwargs):
        sales_details = ast.literal_eval(request.POST['sales_details']) 
        # try:
        sale = Sale.objects.get(id=sales_details['id']) 
        tax_exclusive_total = 0
        tax_exclusive_total = float(sale.grant_total) - float(sale.sales_tax)
        if sales_details['old_customer']:
            old_customer = Customer.objects.get(id=sales_details['old_customer'])
        else :
            old_customer = ''
        if sales_details['customer']:
            customer = Customer.objects.get(id=sales_details['customer'])
            ...
            if old_customer:
                ledger_entries = LedgerEntry.objects.filter(ledger=old_customer.ledger,transaction_reference_number=sale.transaction_reference_no)
            else:
                ledger_entries = LedgerEntry.objects.filter(ledger=customer.ledger,transaction_reference_number=sale.transaction_reference_no)
            if len(ledger_entries) > 0:
                for ledger_entry_data in ledger_entries:
                    if ledger_entry_data.debit_amount >0:
                        ledger_entry_debit_customer = ledger_entry_data
                        ledger_entry_debit_customer.ledger = customer.ledger
            else:
                parent = Ledger.objects.get(name='Sundry Debtors')
                counter_sales_ledger = Ledger.objects.get(name="Counter Sales", parent=parent)
                counter_sales_transaction = Transaction.objects.get(credit_ledger__ledger__account_code=3001,transaction_ref=sale.transaction_reference_no)
                # counter_sales_transaction.debit_ledger = ledger_entry_debit_customer 
                ledger_entries = LedgerEntry.objects.filter(ledger=counter_sales_transaction.debit_ledger.ledger,transaction_reference_number=sale.transaction_reference_no)
                for ledger_entry_data in ledger_entries:
                    if ledger_entry_data.debit_amount >0:
                        ledger_entry_debit_customer = ledger_entry_data
                        ledger_entry_debit_customer.ledger = customer.ledger
            ...
            if old_customer:
                ...
                ledger_entries =  LedgerEntry.objects.filter(ledger=counter_sales_ledger,transaction_reference_number=sale.transaction_reference_no)   
            for ledger_entry_data in ledger_entries:
                if ledger_entry_data.debit_amount >0:
                    ledger_entry_debit_customer = ledger_entry_data
                    ledger_entry_debit_customer.ledger = counter_sales_ledger    
        
        if ledger_entry_debit_customer.debit_amount == None:
            ledger_entry_debit_customer.debit_amount = 0
        ledger_entry_debit_customer.transaction_reference_number = sale.transaction_reference_no
        ledger_entry_debit_customer.debit_amount = float(ledger_entry_debit_customer.debit_amount) - float(sale.grant_total) + float(sales_details['grant_total'])
        ledger_entry_debit_customer.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
        ledger_entry_debit_customer.save()
        
        sales_ledger = Ledger.objects.get(name='Sales')
        ledger_entries = LedgerEntry.objects.get(ledger=sales_ledger,transaction_reference_number=sale.transaction_reference_no)
        # for ledger_entry_data in ledger_entries:
        #     if ledger_entry_data.credit_amount > 0:
        ledger_entry_credit_sales = ledger_entries
        
        ledger_entry_credit_sales.credit_amount = float(ledger_entry_credit_sales.credit_amount) - float(tax_exclusive_total) + float(sales_details['tax_exclusive_total'])
        ledger_entry_credit_sales.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
        ledger_entry_credit_sales.save()
        transaction_1 = Transaction.objects.get(credit_ledger=ledger_entry_credit_sales)
        ...
        if sale.payment_mode != 'credit':
            if sale.payment_mode == 'cash':
                ...
                bank_account_ledger = transaction_1.debit_ledger.ledger.id
                debit_ledger = Ledger.objects.get(id=bank_account_ledger)
            ledger_entries = LedgerEntry.objects.filter(ledger=debit_ledger,transaction_reference_number=sale.transaction_reference_no)
            for ledger_entry_data in ledger_entries:
                if ledger_entry_data.debit_amount > 0:
                    ledger_entry_debit_accounts = ledger_entry_data
            ledger_entry_debit_accounts.debit_amount = float(ledger_entry_debit_accounts.debit_amount) - float(sale.grant_total) + float(sales_details['grant_total'])
            ledger_entry_debit_accounts.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
            ...
            if sales_details['customer']:
                ...
                if old_customer:
                    ...
                    if sale.customer:
                        ledger_entries = LedgerEntry.objects.filter(ledger=sale.customer.ledger,transaction_reference_number=sale.transaction_reference_no)
                if len(ledger_entries) <= 0:

                    counter_sales_transaction = Transaction.objects.get(credit_ledger__ledger__name="Counter Sales",transaction_ref=sale.transaction_reference_no)
                    # counter_sales_transaction.credit_ledger = ledger_entry_credit_customer 
                    ledger_entries = LedgerEntry.objects.filter(ledger=counter_sales_transaction.credit_ledger.ledger,transaction_reference_number=sale.transaction_reference_no)
                    for ledger_entry_data in ledger_entries:
                        if ledger_entry_data.credit_amount >0:
                            ledger_entry_credit_customer = ledger_entry_data
            ...
            ledger_entry_credit_customer.credit_amount = float(ledger_entry_credit_customer.credit_amount) - float(sale.grant_total) + float(sales_details['grant_total'])
            ledger_entry_credit_customer.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
            ...
            if sales_details['customer']:
                ...
                counter_sales_ledger.balance = float(counter_sales_ledger.balance) - float(ledger_entry_credit_customer.credit_amount)
                counter_sales_ledger.save()
            transaction_2 = Transaction.objects.get(debit_ledger=ledger_entry_debit_accounts,transaction_ref=sale.transaction_reference_no)
            transaction_2.credit_ledger = ledger_entry_credit_customer
            transaction_2.debit_ledger = ledger_entry_debit_accounts
            transaction_2.debit_amount = float(transaction_2.debit_amount) - float(sale.grant_total) + float(sales_details['grant_total'])
            transaction_2.credit_amount = float(transaction_2.credit_amount) - float(sale.grant_total) + float(sales_details['grant_total'])
        
        
        credit_stock_ledger = Ledger.objects.get(name="Stock")
        ledger_entries = LedgerEntry.objects.filter(ledger=credit_stock_ledger,transaction_reference_number=sale.transaction_reference_no)
        for ledger_entry_data in ledger_entries:
            if ledger_entry_data.credit_amount:
                ledger_entry_credit_stock = ledger_entry_data
        ledger_entry_credit_stock.credit_amount = float(ledger_entry_credit_stock.credit_amount) - float(tax_exclusive_total) + float(sales_details['tax_exclusive_total'])
        ledger_entry_credit_stock.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
        ledger_entry_credit_stock.save()
        credit_stock_ledger.balance = float(credit_stock_ledger.balance) - float(ledger_entry_credit_stock.credit_amount)
        credit_stock_ledger.save()
        transaction_3 = Transaction.objects.get(credit_ledger=ledger_entry_credit_stock,transaction_ref=sale.transaction_reference_no)
        ...
        if sales_details['bill_type'] == 'Invoice':
            ...
            credit_tax_ledger = Ledger.objects.get(name="Output Vat (Sales)")
            try:
                ledger_entries = LedgerEntry.objects.filter(ledger=credit_tax_ledger,transaction_reference_number=sale.transaction_reference_no)
                for ledger_entry_data in ledger_entries:
                    if ledger_entry_data.credit_amount > 0:
                        ledger_entry_credit_tax_account = ledger_entry_data
                ledger_entry_credit_tax_account.credit_amount = float(ledger_entry_credit_tax_account.credit_amount) - float(sale.sales_tax) + float(sales_details['tax_exclusive_total'])
                ledger_entry_credit_tax_account.date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
                ...
                ledger_entry_credit_tax_account.credit_amount = float(sales_details['grant_total']) - float(sales_details['tax_exclusive_total'])
            ...
            credit_tax_ledger.save()
            try:
                transaction_4 = Transaction.objects.get(credit_ledger=ledger_entry_credit_tax_account,transaction_ref=sale.transaction_reference_no)
            ...
            transaction_4.payment_mode = sales_details['payment_mode']
            if sales_details['payment_mode'] != 'credit':
                if sales_details['payment_mode'] == 'cheque':
                    transaction_4.bank_name = sales_details['bank_name']
                    transaction_4.branch = sales_details['branch']
                elif sales_details['payment_mode'] == 'card':
                    transaction_4.bank_name = sales_details['bank_name']
                    transaction_4.card_no = sales_details['card_no']
                    transaction_4.card_holder_name = sales_details['card_holder_name']
            transaction_4.save()
        if sales_details['bill_type'] == 'Receipt' and sale.bill_type == 'Invoice':
            credit_tax_ledger = Ledger.objects.get(name="Output Vat (Sales)")
            ledger_entries = LedgerEntry.objects.filter(ledger=credit_tax_ledger,transaction_reference_number=sale.transaction_reference_no)
            for ledger_entry_data in ledger_entries:
                if ledger_entry_data.credit_amount > 0:
                    credit_tax_ledger.balance = float(credit_tax_ledger.balance) - float(ledger_entry_data.credit_amount)
                    ledger_entry_data.delete()
                    # transaction_4 = Transaction.objects.get(credit_ledger=ledger_entry_data,transaction_ref=sale.transaction_reference_no)
                    # transaction_4.delete()

        transaction_1.transaction_date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
        
        transaction_1.payment_mode = sales_details['payment_mode']
        if sales_details['payment_mode'] != 'credit':
            transaction_2.transaction_date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
            
            transaction_2.payment_mode = sales_details['payment_mode']
        transaction_3.transaction_date = datetime.strptime(sales_details['invoice_date'], '%d/%m/%Y')
        
        transaction_3.payment_mode = sales_details['payment_mode']

        if sales_details['payment_mode'] != 'credit':
            if sales_details['payment_mode'] == 'cheque':
                transaction_1.bank_name = sales_details['bank_name']
                transaction_2.bank_name = sales_details['bank_name']
                transaction_3.bank_name = sales_details['bank_name']
                transaction_1.cheque_number = sales_details['cheque_no']
                transaction_1.cheque_date = datetime.strptime(sales_details['cheque_date'], '%d/%m/%Y')
                transaction_1.branch = sales_details['branch']
                transaction_2.cheque_number = sales_details['cheque_no']
                transaction_2.cheque_date = datetime.strptime(sales_details['cheque_date'], '%d/%m/%Y')
                transaction_2.branch = sales_details['branch']
                transaction_3.cheque_number = sales_details['cheque_no']
                transaction_3.cheque_date = datetime.strptime(sales_details['cheque_date'], '%d/%m/%Y')
                transaction_3.branch = sales_details['branch']
            elif sales_details['payment_mode'] == 'card':
                transaction_1.bank_name = sales_details['bank_name']
                transaction_2.bank_name = sales_details['bank_name']
                transaction_3.bank_name = sales_details['bank_name']
                transaction_1.card_holder_name = sales_details['card_holder_name']
                transaction_1.card_no = sales_details['card_no']
                transaction_2.card_holder_name = sales_details['card_holder_name']
                transaction_2.card_no = sales_details['card_no']
                transaction_3.card_holder_name = sales_details['card_holder_name']
                transaction_3.card_no = sales_details['card_no']
            transaction_2.save()
        transaction_1.save()
        transaction_3.save()
        ...
        if sales_details['customer']:
            try:
                ...
                customer = Customer.objects.get(name=sales_details['customer'])
            sale.customer = customer
        sale.discount = sales_details['discount']
        if sales_details['round_off'] != '':
            sale.round_off = sales_details['round_off']
        ...
        stock_value_amount = 0
        total_tax = 0
        if sales_details['d_items']:
            d_items = sales_details['d_items']
            for d_item in d_items:
                sales_item = SalesItem.objects.get(id=d_item['id'])
                batch_item_obj = BatchItem.objects.get(batch__id=d_item['batch_id'],item__id=d_item['item_id'])
                actual_quantity = calculate_actual_quantity(batch_item_obj.item, sales_item.quantity, sales_item.uom)         
                batch_item_obj.quantity_in_actual_unit = batch_item_obj.quantity_in_actual_unit + float(actual_quantity)
                batch_item_obj.save()
                sales_item.delete()
        for item in sales_items:
        ...
        if sale.payment_mode == 'cheque':
            post_dated_cheque = PostDatedCheque()
            type_name = 'sales'
            post_dated_cheque.set_attributes(type_name,sale)
            delete_post_dated_cheque_entries()
        res = {
            'result': 'ok',
            'message': 'Transaction saved',
            'transaction_reference_no': sale.transaction_reference_no,
...
class InvoiceSearch(View):

    def get(self, request, *args, **kwargs):
        invoice_no = request.GET.get('sales_invoice', '')
        print invoice_no
        if invoice_no:
            sales = Sale.objects.filter(sales_invoice_number__istartswith=invoice_no)
        invoice_no_list= []
        if request.is_ajax():
            for sale in sales:
                invoice_no_list.append({'invoice_no':sale.sales_invoice_number})
            res = {
                'result': 'ok',
...
class EditedSalesEntry(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get('transaction_ref_no'):
            ...
            elif sale.editedreceipt_set.all().count() > 0:
