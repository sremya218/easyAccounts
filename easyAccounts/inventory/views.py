            ...
            for item in items:
                ...
                try:
                    if batch and batch != 'undefined':
                        ...
                        quantity_in_purchase_unit = batch_item.quantity_in_purchase_unit,
                        quantity_in_smallest_unit = batch_item.quantity_in_smallest_unit,
                        purchase_price = batch_item.purchase_price
                ...
                item_data['quantity_in_smallest_unit'] = quantity_in_smallest_unit
                item_data['purchase_price'] = purchase_price
                    ...
                    for item_detail in opening_stock_items:
                        ...
                        if item.smallest_unit == purchase_unit:
                            ...
                            if purchase_unit == 'box':
                                if item.packets_per_box != None:
                                    quantity = float(item_detail['quantity']) * float(item.packets_per_box)
                                ...
                                if item.pieces_per_box != None:
                                    quantity = float(item_detail['quantity']) * float(item.pieces_per_box)
                                    if item.unit_per_piece != None:
                                        quantity = float(quantity) * float(item.unit_per_piece)
                            if purchase_unit == 'packet':
                                if item.pieces_per_packet != None:
                                    quantity = float(item_detail['quantity']) * float(item.pieces_per_packet)
                                    if item.unit_per_piece != None:
                                        quantity = float(quantity) * float(item.unit_per_piece)
                                elif item.unit_per_packet != None:
                                        quantity = float(item_detail['quantity']) * float(item.unit_per_packet)
                            if purchase_unit == 'piece':
                                if item.unit_per_piece != None:
                                    quantity = float(item_detail['quantity']) * float(item.unit_per_piece)
                        ...
                        if batch_item_created:
                            batch_item.quantity_in_purchase_unit = item_detail['quantity']
                            batch_item.quantity_in_smallest_unit = quantity
                            batch_item.uom = purchase_unit
                        else:
                            batch_item.quantity_in_purchase_unit = float(batch_item.quantity_in_purchase_unit) + float(item_detail['quantity'])
                            batch_item.quantity_in_smallest_unit = float(batch_item.quantity_in_smallest_unit) + float(quantity)
            ...
            for item in items:
                ...
                for batch_item in batch_items:
                   item_dict.append({
                        ...
                        'stock': batch_item.quantity_in_smallest_unit, 
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.is_ajax():
            ...
            for item_data in item_list:
                ...
                data.append([item_name, batch_name, item_data['stock'], item_data['uom']])
            if len(data) > 0:
                table = Table(data, colWidths=(150, 150, 100, 80), style=style)
                elements.append(table)
            p.build(elements)  
    ...
    def get(self, request, *args, **kwargs):
        if request.GET.get('category_id', ''):
            ...
            if request.is_ajax():
                ...
                para_style.fontSize = 10
                para_style.fontName = 'Helvetica'
                data.append(['Item', 'Product', 'Batch', 'Stock', 'UOM'])
                table = Table(data, colWidths=(130, 130, 130, 90, 80), style=style)
                table.setStyle([
                            ('FONTSIZE', (0,0), (-1,0), 11),
                            ])  
                elements.append(table)
